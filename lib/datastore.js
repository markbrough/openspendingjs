...
(function ($) {
  ...
  OpenSpending.Search = function (config) {
    ...
    my.entryUrl = function (entry) {
      ...
      return my._config.endpoint + entry.dataset + '/entries/' + entry_id;
    };

    my.entityUrl = function (entity, dimension_name, dataset_name) {
      return my._config.endpoint + dataset_name + '/' + dimension_name + '/' + entity.name;
    };

    my.entityLink = function (entity, dimension_name, dataset_name) {
      var out = $('<div />').append(
      $('<a />').text(entity.label).attr('href', my.entityUrl(entity, dimension_name, dataset_name))).html();
      return out
    };
    ...
    my.ResultWidget = AjaxSolr.AbstractWidget.extend({
      ...
      afterRequest: function () {
        ...
        var out = $.tmpl(this.tmplEntriesList, {
          docs: entries
        });
      ...
      tmplEntriesList: ' \
        <table> \
          <thead> \
            <tr> \
              <th column="time" class="sortable">Date</th> \
          ...
          <tbody> \
            ...
            <tr> \
              <td>${entry["time"]}</td> \
              <td> \
                {{html my.entityLink(entry.from, "from", entry.dataset)}} \
              </td> \
              <td> \
                {{html my.entityLink(entry.to, "to", entry.dataset)}} \
              </td> \
              <td class="num"> \
                ${my.formatCurrency(entry.currency)}${OpenSpending.Utils.formatAmountWithCommas(entry.amount, 2)} \
              </td> \
              <td> \
                <a href="${my.entryUrl(entry)}">more &raquo;</a> \
              </td> \
            </tr> \
            {{/each}} \
          </tbody> \
